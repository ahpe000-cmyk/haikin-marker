...
const ChatHeader = ({ active, brain, brainLoading, onSyncBrain, onOpenSettings, onClearChat, isMobile }) => (
  <header className={"flex items-center shrink-0 relative " + (isMobile ? "gap-2 px-3 py-2.5" : "gap-3 px-6 py-3")} style={{ background: CARD, borderBottom: "1px solid " + INK_SOFT }}>
    ...
    <button
      ...
      className="px-3.5 py-2 text-xs font-semibold whitespace-nowrap shrink-0 transition-all cut-sm"
      style={{ background: brainLoading ? INK_SOFT : SHU, color: brainLoading ? TEXT_DIM : INK, filter: brainLoading ? "none" : "drop-shadow(0 0 6px " + SHU + "88)", letterSpacing: "0.04em" }}
    >
      {brainLoading ? "同期中…" : (isMobile ? "同期" : "共有脳を同期")}
    </button>
    <button
      onClick={onOpenSettings}
      className="px-3.5 py-2 text-xs font-medium whitespace-nowrap shrink-0 transition-all cut-sm"
